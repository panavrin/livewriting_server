...
var insertInteractionTrace = function(db, data, callback){
  data.servertime = (new Date()).getTime();
  db.collection('Interaction Trace').insertOne( data, function(err, result) {
    if(DEBUG)console.log("inserting: " + data.ip);
    assert.equal(err, null);
    if(DEBUG)console.log("Inserted a document into the Interaction Trace. : " + data._id);
    if (callback)callback();
  });
};
...
var retreiveDocsList = function(db, offset, num, email,editor_type){
    var cursor;
    var options = {
    "limit": num,
    "skip": offset,
  ...
  var query = {};
  if(email){
    query = {'useroptions.email':email,'editor_type':editor_type };
  }

  try {
    cursor = db.collection('Articles')
    .find(query
          ,{useroptions:true,_id:true, localEndtime:true, servertime:true, editor_type:true}
          ,options);
  } catch (err) {
    console.log(err);
  }
...
//app.get('/.well-known/pki-validation/53399E771F6778730AA06FF0E38D9DCA.txt', function(req,res) {
//	res.sendFile("/root/.well-known/pki-validation/53399E771F6778730AA06FF0E38D9DCA.txt");
//});
...
app.post('/loginfb', passport.authenticate('facebook', {
    successRedirect : '/profile', // redirect to the secure profile section
    failureRedirect : '/error-login', // redirect back to the signup page if there is an error
    failureFlash : true // allow flash messages
}));
...
//Posting the interaction tracing
app.post('/interactiontracing', function(req,res) {
  CommonManager.getPostData(req, res, function(data) {
    console.log(data);
    data = JSON.parse(data);
    insertInteractionTrace(app.locals.db, data, function(){
      console.log("interaction traced", data._id);
      res.writeHead(200, {'Content-type': 'application/json'});
...
app.post("/getlist", isLoggedIn, function(req,res){
  CommonManager.getPostData(req, res, function (data) {
      var list = retreiveDocsList(app.locals.db, data.offset, data.num,req.user.local.email,data.editor_type);
    list.toArray(function(err, docs){
      if(err){
        res.writeHead(200, {'Content-type': 'application/json'});
        res.end(err);
