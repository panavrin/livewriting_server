...
var retreiveArticle = function(db, aid){
  var promise;
  try {
    promise = db.collection('Articles').findOne({$or:[ {'_id':mongoose.Types.ObjectId(aid)}, {'aid' : aid} ]});
  } catch (err) {
    console.log(err);
  }
  return promise;
...
var retreiveDocsList = function(db, email, offset, num){
  var cursor;
  var options = {
    "limit": num,
    "skip": offset,
    "sort":  [['useroptions.servertime','desc']]
  };

  try {
    cursor = db.collection('Articles')
    .find({'useroptions.email':email}
          ,{useroptions:true,_id:true, localEndtime:true}
          ,options);
  } catch (err) {
    console.log(err);
  }
...
app.post("/getlist", isLoggedIn, function(req,res){
  CommonManager.getPostData(req, res, function (data) {
    var list = retreiveDocsList(app.locals.db,req.user.local.email, data.offset, data.num);
